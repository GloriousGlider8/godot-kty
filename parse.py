		...
		match type_[0]:
			...
			case "s" if type_ == "str":
				if field["encoding"].removesuffix("le") not in ("ascii", "utf-8", "utf-16", "utf-32"):
					...
					tmp = ""
					if inrepeat:
						tmp = f'\t\tfile.store_buffer({id_}[i].to_{"".join(field["encoding"].removesuffix("le").split("-"))}_buffer()) # {type_}'
					else:
						tmp = f'\tfile.store_buffer({id_}.to_{"".join(field["encoding"].removesuffix("le").split("-"))}_buffer()) # {type_}'
					if field["encoding"].endswith("le") and ksy["meta"].get("endian", "le") == "be":
						tmp = "\tfile.big_endian = false\n" + tmp + "\n\tfile.big_endian = true"
					lines.append(tmp)
			case "s" if type_.removeprefix("s").removesuffix("le") in ("1", "2", "4", "8"):
